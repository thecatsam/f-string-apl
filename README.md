...
value of the load-time parameter (global variable) `FUTURES` includes
the character scalar `'⍥'`, e.g. `FUTURES← '⍥'`, then this fun *future*
feature will be recognized within code fields, without impacting other
capabilities of `⍥`. Right now, there are no further future features.
