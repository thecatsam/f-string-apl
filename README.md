...
2.  From your Dyalog session, enter: <br> `]load ∆F -target=⎕SE`<br>
    ...
    `∆FUtils`, containing `∆F` and supporting services and objects.
3.  By default, the target namespace (<code>*⎕SE.∆FUtils*</code>) will
    be added to the end of `⎕PATH`, if not already defined there.
4.  **∆F** may be called as is, i.e. as `∆F`, or fully specified as
    `⎕SE.∆FUtils.∆F`.
5.  You may always choose to create an alias to **∆F** in your chosen
    namespace (say, `mydir`), e.g. via `mydir.∆F← ⎕SE.∆FUtils.∆F`.
