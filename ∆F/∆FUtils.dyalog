...
⍝ Requires ⍙Gen_LegacyAplAN to define ∆VSET, so is defined after it (see below).
...
⍝ Order "break" chars roughly by frequency, high to low. 
  cfBrkList←  dq sq esc lb rb dol omUs ra da pct libra   
...
⍝ NOTE: We are creating multiline objects using the old method for compatibility with Dyalog 19 etc.
  ∇ {ok}← ⍙Load_Shortcuts 
    ; scUser; scA2; scB2; scC2; scD2; scÐ2; scF2; scJ2; scM2; scQ2; scS2; scT2; scW2; scCD2  
    ; XR ;HT 
  ...
  ⍝ Box
    B← XR scB2← HT   ' ⎕THIS.B ' '{⎕ML←1⋄d←|≡⍵⋄⍺←0⋄⍺⎕SE.Dyalog.Utils.disp⊂⍣(1≥d),⍣(0=d)⊢⍵}' 
  ⍝ Commas (Numeric ~)
      ⎕SHADOW 'cCod'  
      cCod←  '{'
      cCod,←   '⎕IO ⎕ML←0 1⋄'
  ...
  :Section Futures
    ...
    ⍝ See Section Future_CircleDiaeresis
      ⎕SHADOW 'cdFut' 'Note' ⋄ cdFut← 0 
      Note← { ~⊃⌽⍵: _← ⍬
        _← '>>>>> Future Feature ', ⍺,': '
        _,'abled',⍨ 'Dis' 'En'⊃⍨ ⊃⍵
      } 
      :If cd∊ FUTURES ⋄ :Andif 0≠ ≢cdStr← Import_CircleDiaeresis 1
          cdFut← 1                ⍝ If we can't import, we just ignore ⍥.
        ⍝ Adam B's "future"  ⍥ with depth operator extension. We call it 'CD' internally. 
          ⍎'CD←',cdStr 
          scCD2←  HT ' ⎕THIS.CD ' cdStr
      :Else
          scCD2← 2⍴ cd 
      :Endif 
       'CircleDiaeresis (⍥) Depth-Extension' Note cdFut VERBOSE_LOADTIME
      ⎕SHADOW 'rsuFut' ⋄ rsuFut← 0 
      :If rsu∊ FUTURES 
          rsuFut←1 
          scSel2← (2⍴⊂ '(⊂⍛⌷)') (2⍴⊂ '(⌷⍤0 99)')⊃⍨ APL_VERSION< 20  ⍝ (⊂⍛⌷) seems faster.
      :Else 
          scSel2← 2⍴ rsu 
      :EndIf 
  ...
  :EndSection Futures 
    ...
    scCodeTbl← ↓⍉↑scA2 scB2 scC2 scD2 scF2 scJ2 scQ2 scS2 scT2  scW2   scÐ2 scM2 scCD2 scSel2 
...
⍝ See Section CircleDiaeresis
∇ {cdStr}← Import_CircleDiaeresis flag; cdNm 
  cdNm← '∆F/CircleDiaeresis.dyalog'
  :If flag   
      ⎕SHADOW 1⊃⎕NPARTS cdNm 
      2 ⎕FIX 'file://',cdNm                     ⍝ See MIT License in file CircleDiaeresis.dyalog.
      cdStr← {                                  ⍝ Generate CircleDiaeresis code string.
          bdy← 1↓ ¯1↓ ⎕NR ⍵                     ⍝ Remove first "nm←{" and last "}" lines.
          bdy← '''[^'']*''' '^\h*(⋄?)' '\h*⍝.*'  ⎕R '\0' '\1' '' ⊣bdy 
           '{','}',⍨ 1↓ ∊'⋄',¨ bdy 
      } 1⊃ ⎕NPARTS cdNm 
  :Else 
