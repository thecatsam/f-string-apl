...
⍝ FUTURES: 
⍝ - If it contains ⍥, '⍥'∊FUTURES,  ⍥ is extended to include the Depth operation, i.e.
⍝   to select subarrays based on depth (just as ⍤ selects subarrays based on Rank).
⍝ - If not, only the standard behaviours of ⍥ (in versions 19 and 20) are enabled.
...
⍝ ∘ CFQS:     (code field) quoted strings
  ...
  ⍝ ScanCF will delete runs of leading and trailing blanks from output code; internal runs will remain.
    ...
    ⍝ This next line will never be executed unless ('⍥'∊FUTURES).
      c= cdCh:  (pfx, scCD) ∇ w                        ⍝ Adam B's CircleDiaeresis (optional).
...
⍝   cfL       -   code field running length (for SDCFs). Set in dfn ScanAll.
  ...
  ⍝ This must follow the ordering specified there EXACTLY.  
    (scA scB scC scD scF scJ scQ scS scT scW scÐ scM scCD)← ûsr.inline⊃ scCodeTbl   
...
⍝ Other basic characters
  sp lb rb lp rp dol omUs ra da pct libra cdCh← ' {}()$⍹→↓%£⍥' 
...
⍝ If (~'⍥'∊FUTURES), we will (correctly) never see '⍥' in Code Fields  
  cfBrkList,← (cdCh/⍨ '⍥'∊FUTURES), ∊qtsL 
  tfBrkList← esc lb                 
...
⍝ NOTE: We are creating multiline objects using the old method for compatibility with Dyalog 19 etc.
  ...
  :Section CircleDiaeresis 
    ...
    ⍝ See Section Future_CircleDiaeresis
      ⎕SHADOW 'cdFut' ⋄ cdFut← 0 
      :If '⍥'∊ FUTURES
      :Andif 0≠ ≢cdStr← Import_CircleDiaeresis 1
      ...
      :Endif 
      cdFut { ~⍵: _← ⍬
        ⎕← '>>> CircleDiaeresis (⍥) Depth-Extension Future Feature ','abled',⍨ 'Dis' 'En'⊃⍨⍺ 
      } VERBOSE_LOADTIME
  :EndSection CircleDiaeresis 

    scCodeTbl← ↓⍉↑scA2 scB2 scC2 scD2 scF2 scJ2 scQ2 scS2 scT2  scW2   scÐ2 scM2 scCD2 
...
⍝ Show the following special globals in this namespace.
  ⍙ShowGlobalsIf←{  ⍝ 
     ...
     These,← 'LIB_ACTIVE'          'SIGNAL_LIB_ERRS' 'LIB_PARM_FI'  
     These,← 'LIB_USER_FI'         'LIB_SRC_FI'      
     These,← 'OPTS_KW'             'OPTS_DEFval'      
     These,← 'QUOTES_SUPPLEMENTAL' 'TRAP_ERRORS'  
     These,← 'VERBOSE_LOADTIME'    'VERBOSE_RUNTIME' 'VERSION' 'FUTURES'
    ~⍵: _←1 0⍴0  
