...
⍝ KEEP_SRC_CM: Set to 1 or 0 in ∆F.dyalog 
  KEEP_SRC_CM← ⎕NULL 
...
⍝ ⍙Export_∆F : rc← ∇ targNm destNs keepCm lockFn 
...
⍝    ∘ we remove comments and comment lines.
  ∇ {targNm}← ⍙Export_∆F (targNm destNs keepCm lockFn) 
    ; nm; ss
    ; Fix; Freeze; QCom; NoEL; QLock 

    ss←  ⊂'∆FSrc'              targNm 
    ss,← ⊂'__THIS__'           ∆THIS  
    ss,← ⊂'__OUTER__\.'        ('##.' ''⊃⍨ destNs=⎕THIS) 
    ss,← ⊂'__TRAP_ERRORS__'    (('⍬0'⊃⍨ TRAP_ERRORS), (¯40↑ '⍝ TRAP_ERRORS='), ⍕TRAP_ERRORS)
    ss,← ⊂'result'             'rësûlt∆F'
    ss,← ⊂'opts'               'öôpts∆F'
    ss,← ⊂'args'               'äârgs∆F'
   
    QCom←   keepCm∘{ ⍺: ⍵ ⋄ '''[^'']*''' '(^\s*)?⍝(?!\!).*$' ⎕R '\0' ''⊢ ⍵ }    
    Freeze← ss∘{ src snk← ↓⍉↑⍺ ⋄ src ⎕R snk⍠ 'UCP' 1 ⊢ ⍵ } 
    NoEL←   { ⍵/⍨ 0≠≢¨⍵ }                             ⍝ NoEL: No Empty Lines  
    Fix←    destNs∘{11:: '' ⋄ ⊃2 ⍺.⎕FIX ⍵}
    QLock←  destNs.⎕LOCK⍣lockFn

    :If targNm≡ nm← Fix NoEL Freeze QCom⊢ 62 ⎕ATX '∆FSrc' 
      {} QLock nm 
...
⍝ NOTE: We are creating multiline objects using the old method for compatibility with Dyalog 19 etc.
  ...
  ⍝ Loading the help html file...
    ...
    :EndTrap 
  ∇
  ∇ {loadLib}← ⍙Load_LibAuto (fi loadLib keepCm)
    ; how 
    how← ' from "',fi,'" into "','"',⍨∆THIS 
    :If loadLib=0
        ⎕←'❗❗❗ WARNING: Library autoload services were not loaded',how
        ⎕←'✅✅✅ LIB_ACTIVE is set to ',(⍕LIB_ACTIVE),' in ∆F/∆FUtils.dyalog'
    ...
    :TRAP 22 
        :If keepCm 
           ⎕FIX 'file://',fi
        :Else 
           ⎕FIX⍠ 'FixWithErrors' 0⊣{ ⍵/⍨ 0≠≢¨⍵ } '''[^'']*''' '\h*⍝.*$' ⎕R '&' ''⊢ ⊃⎕NGET fi 1 
        :EndIf 
        :If VERBOSE_LOADTIME 
...
⍝ Show the following special globals in this namespace.
  ⍙ShowGlobalsIf←{   
     ...
     GLOBALS,← 'QUOTES_SUPPLEMENTAL' 'TRAP_ERRORS'     'KEEP_SRC_CM'  
...
⍝ ====================================================================================
  ∇ ok← Initialise; keepCm  
    :Trap ok← 0   
        :If APL_VERSION < 20
            ⎕←↑3⍴⊂'❌❌❌ This version of ∆F requires Dyalog 20 or later'
            ⎕SIGNAL 911 
        :EndIf  
        ⍙Export_∆F '∆F' ##  0 0                       ⍝ Keep comments? (0→NO); lock? (0→NO).
        ⍙Load_Shortcut_Calls
        ⍙Load_Help HELP_HTML_FI
        KEEP_SRC_CM←  ⎕SE.∆F__KEEP_SRC_CM__
        ⍙Load_LibAuto LIB_SRC_FI LIB_ACTIVE KEEP_SRC_CM   
